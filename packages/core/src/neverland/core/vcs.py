...
README = (
    ...
    "- `todos/`: every todo you have not completed, whatever its state\n"
    "- `done/`: completed todos (archive)\n"
    "- `projects/`: outcomes requiring more than one action\n"
    "- `plans/`: one file per day, the working set you picked\n"
    "- `config.toml`: areas / contexts shared across devices\n"
)
