...
the core source and asserts no module reaches into ``neverland.cli`` (or a future
``neverland.server``). The ruff banned-api rule catches the same thing at edit
...
CORE_SRC = Path(__file__).resolve().parents[1] / "src" / "neverland" / "core"
FORBIDDEN = ("neverland.cli", "neverland.server")
