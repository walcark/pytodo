...
the split into two distributions (`neverland-core` + `neverland-cli`). Only the server
(phase 6) remains; `neverland-server` is not built yet, so the split is two
...
**neverland owns `config.toml`, so regenerating it is fine.** `tomllib` is
...
5. **Split the packages** (done, for core + cli): `packages/core` and
   `packages/cli`, PEP 420 namespace, pixi workspace. `neverland-server` is left
...
Distributions in one monorepo under `packages/`, sharing the `neverland` namespace
(PEP 420: **no** `neverland/__init__.py` in any of them). Two are built today
(`core`, `cli`); `server` is the phase-6 target, listed here for the shape.

```
neverland-core     neverland/core/    todo, project, plan, vocabulary, settings,
                                store, vcs, service   (+ __main__: sync flush)
                                deps: pyyaml
neverland-cli      neverland/cli/     main, prompt, view
                                deps: neverland-core, typer, rich
neverland-server   neverland/server/  app, api, web/, poller       (not built yet)
                                deps: neverland-core, fastapi, uvicorn
```
